...
#include "marsha/kalman_filter.h"
...
class TrajectoryPredictor {
    private:
        ...
        void position_callback(const geometry_msgs::Pose& msg) {
            ...
            if (kalman_initialized < 2) {
                initial_position = msg_to_v3f(msg.position);
                initial_time = ros::Time::now();
                previous_time = ros::Time::now();
                kalman_initialized++;
            }
            else if (kalman_initialized == 2) {
                position_buffer.push_back(msg_to_v3f(msg.position));
        ...
        {
            Vector3f base_position = Vector3f(0.5, 0, 0);

            Vector3f curr_pos = kf->current_position() - base_position;
            ...
            // Test if not moving
            if (curr_vel[0] + curr_vel[1] + curr_vel[2] < 0.0001)
            {
                geometry_msgs::Point position_msg;
                position_msg.x = curr_pos[0];
                position_msg.y = curr_pos[1];
                position_msg.z = curr_pos[2];
                res.position = position_msg;
                res.predicted_time.data = previous_time + ros::Duration(2); // Enough time to move to pre_grasp
                return true;
            }

            // Calculate time closest to end effector
            Searcher searcher(curr_pos, kf->current_velocity(), a);
    ...
    public:
        ...
        TrajectoryPredictor(ros::NodeHandle *nh) {
            object_position_subscriber = nh->subscribe("/mujoco/object_pose", 1, &TrajectoryPredictor::position_callback, this);
            reset_subscriber = nh->subscribe("reset", 1, &TrajectoryPredictor::reset_callback, this);    

            prediction_ready_service = nh->advertiseService("/trajectory_predictor/ready", &TrajectoryPredictor::readyToPredict, this);
            predict_position_service = nh->advertiseService("/trajectory_predictor/predict", &TrajectoryPredictor::predictPosition, this);
            observation_service = nh->advertiseService("/trajectory_predictor/observe", &TrajectoryPredictor::observe, this);
