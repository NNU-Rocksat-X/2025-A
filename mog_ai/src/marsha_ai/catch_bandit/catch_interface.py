...
class CatchInterface(RosInterface):
    ...
    def __init__(self, arm_ns="", training=False):
    ...
    def setup_services(self, arm_ns):
        ...
        rospy.wait_for_service('/' + arm_ns + '/grasp_cmd')
        self.grasp_cmd = rospy.ServiceProxy('/' + arm_ns + '/grasp_cmd', MoveCmd)

        rospy.wait_for_service('/' + arm_ns + '/ARM1/is_grasped')
        self.is_grasped = rospy.ServiceProxy('/' + arm_ns + '/ARM1/is_grasped', GraspDetect) # This should be changed to a trigger srv
        ...
        self.predict_position = rospy.ServiceProxy('/trajectory_predictor/predict', PredictPosition)
    ...
    def perform_action(self, action):
        try:
            ...
            print("predicted time", prediction.predicted_time.data)
            print("delta time", (prediction.predicted_time.data - rospy.get_rostime()).secs)
            #grasp_time = Time(prediction.predicted_time.data - rospy.Duration(action[4]))
            grasp_time = Time(prediction.predicted_time.data)
            ...
            catch_success = self.is_grasped().is_grasped
